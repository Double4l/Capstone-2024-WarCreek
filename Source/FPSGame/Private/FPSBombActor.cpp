...
void AFPSBombActor::NMC_Explode_Implementation()
{
	...
	UGameplayStatics::ApplyRadialDamage(this, 100, GetActorLocation(), 250, UBombDamageType::StaticClass(), TArray<AActor*>(), this, GetInstigatorController());
...
void AFPSBombActor::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	GetWorldTimerManager().SetTimer(Explode_TimerHandle, this, &AFPSBombActor::NMC_Explode, ExplodeDelay, false);
}

void AFPSBombActor::Hold(USkeletalMeshComponent* HoldingComponent, APlayerController* PlayerController)
{
	//IF HoldingComponent
	if (HoldingComponent)
	{
		SetInstigator(PlayerController->GetPawn());
